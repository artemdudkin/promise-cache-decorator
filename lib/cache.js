...
function put(opt, id, value) {
  ...
  return value
         .then(res => {
            ...
            let sv = _storage.save(id, _serialize(value));
            //storage.save SHOULD return Promise but CAN return anything
            if (!sv || typeof sv.then != 'function') sv = Promise.resolve(sv);
            sv.catch(err=>{
              console.error("ERROR: cannot save("+id+") to storage: " + err);
            })
            .then(res=>{
              //we will not return result of storage.save
              return
            });
           return sv;
         })
...
function invalidate_all() {
  for (var i in _cached) remove(undefined, i)
}
